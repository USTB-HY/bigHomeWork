        ...
        <div class="bread">
          ...
          <ul class="fl sui-tag">
            <li class="with-x" v-if="searchParams.categoryName">{{searchParams.categoryName}}
                <i @click="removeBreadTagCategoryName">×</i>
            </li>
            <li class="with-x" v-if="searchParams.keyword">{{searchParams.keyword}}
                <i @click="removeBreadTagKeyword">×</i>
            </li>
...
export default {
  ...
  methods: {
    removeBreadTagCategoryName(){
        this.searchParams.categoryName = undefined //修改为undefined发送请求时不会发出可不占用带宽
        ...
        let location = {
            name: "Search",
            params: this.$route.params, //保留params参数，只去除类名
        }
        this.$router.push(location)
    },
    removeBreadTagKeyword() {
        this.searchParams.keyword = undefined //修改为undefined发送请求时不会发出可不占用带宽
        this.searchParams.category1Id = undefined
        this.searchParams.category2Id = undefined
        this.searchParams.category3Id = undefined
        this.getData()
        let location = {
            name: "Search",
            query: this.$route.query, //保留params参数，只去除类名
        }
        this.$router.push(location)
        //采用全局事件总线删除Header搜索栏里的关键字
        this.$bus.$emit('clearInput')
    },
    removeBreadTagTradeMark() {
  ...
  beforeMount() {
    Object.assign(this.searchParams,this.$route.query,this.$route.params) //合并相同属性
  },
  mounted() {
    this.getData()
